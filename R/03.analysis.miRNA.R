...
write.table(res_ordered,file="hcc_lulab.sequentialMap.homer.merged.clean.impute.CtrlvsHCC.edgeR.tsv",sep="\t", quote = F, row.names=FALSE)
...
write.table(top_ordered,file="hcc_lulab.sequentialMap.homer.merged.clean.impute.CtrlvsHCC.DESeq2.tsv",sep="\t",quote = F, row.names=T)
...
outlist <- list() 
pdf("reads.qc.impute.remove_confounders.selectgenes.roc.pdf")
for (i in 1:length(genelist)){
title <- names(genelist[i])
...
write.table(outmx, "./hcc_lulab.sequentialMap.homer.merged.ml.stat.txt", sep="\t",quote=F, row.names=F, col.names=T)
