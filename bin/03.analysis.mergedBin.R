# filter matrix
awk 'FNR == 1{print;next}{for(i=3;i<=NF;i++) if($i > 0){print;next}}' hcc_lulab.sequentialMap.homer.merged.binned.mx > hcc_lulab.sequentialMap.homer.merged.binned.filtered.mx
...
raw_mx <- read.table("hcc_lulab.sequentialMap.homer.merged.binned.filtered.mx", sep = "\t")
...
# save the data
saveRDS(reads.qc, file = "hcc_lulab.sequentialMap.homer.merged.clean.binned.rds")

##---------------------
## Imputation
library("scImpute")
write.csv(counts(reads.qc), "hcc_lulab.sequentialMap.homer.merged.clean.binned.rds.csv")

reads.qc <- readRDS("hcc_lulab.sequentialMap.homer.merged.clean.binned.rds")
sampleLables <- c()
...
saveRDS(reads.qc.impute, file = "hcc_lulab.sequentialMap.homer.merged.clean.impute.binned.rds")
...
write.table(res_ordered,file="hcc_lulab.sequentialMap.homer.merged.clean.impute.binned.CtrlvsHCC.DESeq2.tsv",sep="\t", quote = F, row.names=FALSE)
...
write.table(top_ordered,file="hcc_lulab.sequentialMap.homer.merged.clean.impute.binned.CtrlvsHCC.edgeR.tsv",sep="\t",quote = F, row.names=T)
...
genelist <- list(edgeR.diffexp.top.200.list, edgeR.diffexp.fdr05.list, edgeR.diffexp.fdr001.list, deseq2.diffexp.top.200.list, deseq2.diffexp.fdr05.list, deseq2.diffexp.fdr001.list, reads.qc.impute.HVG_genes.top.200, reads.qc.impute.HVG_genes.fdr05, reads.qc.impute.HVG_genes.fdr001, reads.qc.impute.top_hvgs.200, reads.qc.impute.top_hvgs.fdr05, reads.qc.impute.top_hvgs.fdr001, reads.qc.impute.top_m3drop.top.200, reads.qc.impute.top_m3drop.fdr05, reads.qc.impute.top_m3drop.fdr001, Cor_genes.top.200, Cor_genes.top.500, Cor_genes.top.1000, PCA_genes.top.200, PCA_genes.top.500, PCA_genes.top.1000)
names(genelist) <- c("edgeR.diffexp.top.200.list", "edgeR.diffexp.fdr05.list", "edgeR.diffexp.fdr001.list", "deseq2.diffexp.top.200.list", "deseq2.diffexp.fdr05.list", "deseq2.diffexp.fdr001.list", "reads.qc.impute.HVG_genes.top.200", "reads.qc.impute.HVG_genes.fdr05", "reads.qc.impute.HVG_genes.fdr001", "reads.qc.impute.top_hvgs.200", "reads.qc.impute.top_hvgs.fdr05", "reads.qc.impute.top_hvgs.fdr001", "reads.qc.impute.top_m3drop.top.200", "reads.qc.impute.top_m3drop.fdr05", "reads.qc.impute.top_m3drop.fdr001", "Cor_genes.top.200", "Cor_genes.top.500", "Cor_genes.top.1000", "PCA_genes.top.200", "PCA_genes.top.500", "PCA_genes.top.1000")

save(genelist,file="genelist.binned.Rdata")

pdf("reads.qc.impute.remove_confounders.selectgenes.binned.heatmap.pdf")
...
outlist <- list() 
pdf("reads.qc.impute.remove_confounders.selectgenes.binned.glm.roc.pdf")
for (i in 1:length(genelist)){
title <- names(genelist[i])
...
write.table(outmx, "./hcc_lulab.sequentialMap.homer.merged.binned.glm.stat.txt", sep="\t",quote=F, row.names=F, col.names=T)
